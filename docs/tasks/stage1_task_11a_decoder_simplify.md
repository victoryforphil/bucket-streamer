# Task 11a: Decoder API Simplification

## Goal
Simplify decoder to a single byte-offset based API that matches the protocol's frame addressing scheme (IRAP offset + frame byte offset).

## Dependencies
- Task 08: AVIOContext Spike
- Task 09: Decoder Module (replaces parts of it)

## Design Decisions

1. **Single decode method** - Replace multiple APIs (`decode_frames`, `decode_all_frames`) with one `decode_frame()` that takes byte offsets
2. **Match protocol semantics** - Use same addressing as `RequestFrames`: IRAP byte offset + target frame byte offset
3. **Sequential decode internally** - H.265 requires decoding from IRAP; we decode until we hit target offset
4. **No frame index tracking** - Frame indices are client-side concerns; server only needs byte offsets
5. **Packet position matching** - Match target frame by comparing FFmpeg packet positions during decode

## Files to Modify

```
crates/bucket-streamer/src/pipeline/decoder.rs   # Simplify API
```

## Steps

### 1. Replace decoder.rs implementation

Remove `decode_frames`, `decode_all_frames`, `decode_up_to`. Replace with single focused API:
...
mod tests {
    ...
    fn test_decoder_reuse() {
        ...
        let frame1 = decoder.decode_frame(&data, first_offset).expect("First decode failed");
        let frame2 = decoder.decode_frame(&data, first_offset).expect("Second decode failed");
    ...
    fn test_yuv420p_format() {
        ...
        let mut decoder = Decoder::new(&data).expect("Decoder creation failed");
        let frame = decoder.decode_frame(&data, first_offset).expect("Decode failed");
    ...
    fn benchmark_frame_decode() {
        ...
        let first_offset = {
            ...
            let path = std::env::var("TEST_VIDEO_PATH")
                .unwrap_or_else(|_| "data/test.h265.mp4".to_string());
            let ictx = ffmpeg::format::input(&path).unwrap();
            let stream_idx = ictx.streams().best(ffmpeg::media::Type::Video).unwrap().index();
            let mut ictx = ffmpeg::format::input(&path).unwrap();
            ictx.packets()
...
- [ ] Single `decode_frame(video_data, target_offset)` method replaces multiple APIs
- [ ] Returns frame matching packet at `target_offset`
- [ ] `FrameNotFound` error when offset doesn't match any packet
- [ ] Decoder can be reused across multiple decode calls
- [ ] Output is YUV420P format matching TurboJPEG expectations
...
```bash
# Run decoder tests
cargo test -p bucket-streamer decoder -- --nocapture

# Run benchmarks
cargo test -p bucket-streamer --release -- --ignored --nocapture benchmark_frame
```
...
The client (via offset JSON from `repo-cli convert`) knows exact byte positions of each frame. Using byte offsets directly:
- Eliminates need for server to parse/maintain offset JSON
- Matches protocol semantics exactly
- Simplifies the API surface

### Packet Position Matching

FFmpeg's `packet.position()` returns the byte offset of the packet in the source file. By comparing this to `target_offset`, we can identify exactly which frame the client requested.

### Sequential Decoding Requirement

H.265 uses inter-frame prediction. To decode frame N, we must decode all frames from the nearest keyframe. The client provides frame requests grouped by IRAP, allowing the server to:
1. Load video data starting from IRAP offset
2. Decode sequentially until reaching target offset
3. Return just the requested frame

### Thread Safety

`Decoder` is not `Send`/`Sync` due to FFmpeg internals. In async code:
```rust
let frame = tokio::task::spawn_blocking(move || {
    decoder.decode_frame(&video_data, offset)
}).await??;
```

This pattern is used in Task 11's Session implementation.
