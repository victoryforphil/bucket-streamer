...
## Goal
Implement H.265 decoder with persistent codec context, GOP-based decoding from byte ranges, and proper resource management.
...
1. **Keep format context alive** - Store `AVFormatContext` and `AvioContext` in Decoder to avoid reopening for each decode call
2. **Byte-range input** - Accept valid MP4 structure (headers + GOP mdat section) from fetcher
3. **YUV420P output only** - Optimized for TurboJPEG direct encoding
4. **Initialize scaler upfront** - Create scaler in `new()` rather than lazily
5. **Relative frame indices** - Frame 0 = IRAP keyframe, frame 1 = first P/B frame within GOP
6. **Always flush** - Flush decoder state between decode calls for safety
...
- [ ] `Decoder::new()` initializes from video bytes with scaler
- [ ] `decode_frames()` returns valid YUV420P data for requested indices
- [ ] `decode_all_frames()` decodes entire GOP
- [ ] Frame dimensions and linesize match expected YUV420P format
- [ ] `DecoderError` provides meaningful error variants
- [ ] Decoder can be reused across multiple GOP decodes
- [ ] Decoder flushes state between decode calls
- [ ] `FrameNotFound` error for invalid frame indices
- [ ] Tests pass: `cargo test -p bucket-streamer decoder`
- [ ] Benchmarks show reasonable performance (>20 FPS target)
...
valgrind --leak-check=full ./target/debug/deps/bucket_streamer-* decoder
```

## Context

### Decoder Reuse
Creating an FFmpeg decoder takes ~30ms. By keeping the codec context alive and only recreating the format context per GOP, we save significant overhead.
...
TurboJPEG can encode YUV420P directly without RGB conversion, saving ~2ms per frame.

### GOP Data Format
The fetcher provides complete valid MP4 chunks:
- MP4 headers (moov atom) included
- Relevant mdat portion for the GOP

This simplifies the decoder - it just opens a standard MP4 stream each time. The overhead of including headers (~few KB) is negligible compared to video data.

### Frame Indexing
Frame indices are relative to the GOP:
- Index 0 = IRAP (keyframe)
- Index 1 = First P/B frame
- Index N = Nth frame in decode order

The pipeline maps absolute frame numbers to GOP-relative indices.

### Always Flush Strategy
Flushing between GOP decodes ensures:
- No stale reference frames from previous GOP
- Predictable behavior regardless of previous state
- Simpler debugging when issues occur

The flush cost (~0.1ms) is negligible compared to decode time.

### Thread Safety
`Decoder` is not `Send`/`Sync` due to FFmpeg internals. In the pipeline:
```rust
let frame = tokio::task::spawn_blocking(move || {
    decoder.decode_frames(&gop_data, &[0])
}).await??;
```

### Memory Layout
DecodedFrame stores planes contiguously (not with FFmpeg's stride padding):
```
...
This matches TurboJPEG's expected I420 format.
