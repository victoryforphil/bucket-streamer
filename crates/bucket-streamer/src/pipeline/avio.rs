use bytes::Bytes;
use ffmpeg_sys_next::{
    self as ffi, av_free, av_malloc, avformat_alloc_context, avformat_close_input,
    avformat_find_stream_info, avformat_open_input, avio_alloc_context, avio_context_free,
    AVFormatContext, AVIOContext, AVSEEK_SIZE,
};
use std::ffi::c_void;
...
unsafe extern "C" fn read_packet(opaque: *mut c_void, buf: *mut c_uchar, buf_size: c_int) -> c_int {
    ...
    ptr::copy_nonoverlapping(io.data[io.position..].as_ptr(), buf, to_read);
    io.position += to_read;
    to_read as c_int
}
